...
export async function sendApprovalEmail(to: string, name: string) {
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: 'Sua conta foi aprovada — Frame System 🎉',
    html: `
<!DOCTYPE html>
<html lang="pt-BR">
...
<body style="margin:0;padding:0;background:#0B0C0E;font-family:Inter,sans-serif;">
  <div style="max-width:480px;margin:40px auto;padding:0 16px;">
    ...
    <div style="background:#141618;border:1px solid #1E2124;border-radius:20px;padding:36px 32px;">
      <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#F4F5F0;">Conta aprovada! 🎉</h1>
      <p style="margin:0 0 24px;font-size:14px;color:#6B7280;line-height:1.6;">
        Olá, ${name}! Sua conta no Frame System foi aprovada. Você já pode acessar o painel e começar a configurar sua recepcionista virtual.
      </p>

      <a href="${loginUrl}" style="display:block;text-align:center;background:#00C27C;color:#fff;font-weight:600;font-size:14px;text-decoration:none;border-radius:12px;padding:14px 24px;">
        Acessar o painel
      </a>
