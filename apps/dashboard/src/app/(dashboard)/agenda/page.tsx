...
import { Avatar } from '@/components/ui/finance-primitives'
import { V4Button, V4Card, V4CardPad, V4Metric, V4Page, V4SectionTitle, V4Tag } from '@/components/v4/V4Primitives'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[90vh]"
        ...
        <div className="px-5 py-4 flex gap-3 flex-shrink-0" style={{ borderTop:'1px solid var(--border)' }}>
          <V4Button onClick={onClose} className="flex-1">Cancelar</V4Button>
          <V4Button onClick={handleSave} disabled={saving} variant="primary" className="flex-1">
      ...
      <div className="relative w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden z-10"
        ...
        <div className="px-5 py-4 space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <V4Tag tone={s.tag} dot>{s.label}</V4Tag>
            {appt.modality && (
              <V4Tag>
            ...
            {appt.location_name && (
              <V4Tag><MapPin className="w-3 h-3" /> {appt.location_name}</V4Tag>
            )}
          </div>
        ...
        {appt.status !== 'cancelled' && appt.status !== 'completed' && (
          <div className="px-5 pb-5 flex gap-2">
            {appt.status === 'scheduled' && (
              <V4Button onClick={() => { onUpdate(appt.id, 'confirmed'); onClose() }} className="flex-1">
                <CheckCircle className="w-4 h-4" /> Confirmar
              </V4Button>
            )}
            {(appt.status === 'scheduled' || appt.status === 'confirmed') && (
              <V4Button onClick={() => { onUpdate(appt.id, 'completed'); onClose() }} variant="primary" className="flex-1">
                <CheckCheck className="w-4 h-4" /> Realizado
              </V4Button>
            )}
            <V4Button onClick={() => { onUpdate(appt.id, 'cancelled'); onClose() }} variant="danger">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden z-10"
        ...
        <div className="px-5 pb-5 flex gap-3">
          <V4Button onClick={onClose} className="flex-1">Cancelar</V4Button>
          <V4Button onClick={handleSave} disabled={saving} variant="danger" className="flex-1">
...
export default function AgendaPage() {
  ...
  return (
    <V4Page
      ...
      title="Agenda"
      subtitle={`Semana de ${format(weekStart, 'd')} a ${format(weekEnd, "d 'de' MMMM", { locale: ptBR })} · ${todayCount} consulta${todayCount !== 1 ? 's' : ''} hoje.`}
      actions={
        <>
          <V4Button onClick={prevWeek} className="h-9 px-3"><ChevronLeft className="h-4 w-4" /></V4Button>
          <V4Button onClick={goToday} variant={isCurrentWeek ? 'primary' : 'default'}>Hoje</V4Button>
          <V4Button onClick={nextWeek} className="h-9 px-3"><ChevronRight className="h-4 w-4" /></V4Button>
          <V4Button onClick={() => setBlockDate(new Date())}><Ban className="h-4 w-4" />Bloquear período</V4Button>
          <V4Button variant="primary" onClick={() => setNewApptDate(setHours(startOfWeek(weekStart, { weekStartsOn: 1 }), 9))}>
    ...
    >
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <V4Metric label="Consultas na semana" value={weekTotal} foot={weekRangeLabel} />
        <V4Metric label="Confirmadas" value={weekConfirmed} foot={`de ${weekTotal} agendadas`} tone="good" />
        <V4Metric label="Realizadas" value={weekCompleted} foot="concluídas nesta semana" />
        <V4Metric label="Canceladas" value={weekCancelled} foot="nesta semana" tone={weekCancelled > 0 ? 'bad' : 'default'} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[200px_minmax(0,1fr)]">
        <aside className="hidden lg:flex flex-col gap-5">
          ...
          <V4CardPad>
            <p className="font-mono text-[9px] text-t3 tracking-wider uppercase mb-2">Status</p>
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded flex-shrink-0" style={{ background:'color-mix(in oklab, var(--t3) 35%, transparent)' }} />
        ...
        <V4Card className="overflow-hidden" style={{ height: '600px' }}>
          <WeekView
            weekStart={weekStart}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <V4CardPad>
          <V4SectionTitle title="Próximas 24 horas" subtitle={`${next24h.length} consulta${next24h.length !== 1 ? 's' : ''}`} />
          <div className="space-y-1">
            ...
            {next24h.map(a => {
              const s = STATUS[a.status] || STATUS.scheduled
              return (
                <div key={a.id} className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-[var(--raised)]">
        ...
        <V4CardPad>
          <V4SectionTitle title="Bloqueios" subtitle={`${sortedBlocks.length} nesta semana`} />
          <div className="space-y-1">
            ...
            {sortedBlocks.map(b => {
              ...
              const e = parseISO(b.ends_at)
              return (
                <div key={b.id} className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-[var(--raised)]">
